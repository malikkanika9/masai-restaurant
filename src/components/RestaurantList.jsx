...
export default class RestaurantList extends React.Component {
  ...
  render() {
    ...
    return (
      <>
        <h2> Masai Restaurant </h2>
        {new Array(totalPages).fill(0).map((a, i) => (
          <button key={i} onClick={() => this.handlePageChange(i + 1)} style={{backgroundColor:'White',marginLeft:"20px"}}>
            {i + 1}
          </button>
        ))}
        <div>
          {[4, 3, 2, 1, 0].map((rating) => (
            <button key={rating} onClick={() => this.handleRating(rating)} style={{backgroundColor:'White',marginLeft:"20px"}}>
        ...
        <div>
          {["asc", "desc"].map((order) => (
            <button key={order} onClick={() => this.handleSort(order)} style={{backgroundColor:'white',marginLeft:"20px"}}>
              {order}
            </button>
          ))}
        </div>
